...
router.post('/upload', uploadMemory.array('images', 50), async (req, res) => {
  try {
    ...
    for (const file of files) {
      const fileUrl = uploadService.saveFile(undefined, file.buffer, file.originalname, { isGallery: true });
    ...
    res.status(201).json(Array.isArray(results) && results.length === 1 ? results[0] : results);
  } catch (err) {
