...
@Component({
  selector: 'app-video-poster-selector',
  standalone: true,
  imports: [CommonModule],
  templateUrl: './video-poster-selector.component.html',
  styleUrl: './video-poster-selector.component.scss'
})
export class VideoPosterSelectorComponent implements OnInit {
  ...
  extractThumbnailAtCurrentTime(): void {
    ...
    this.ordersService.extractVideoThumbnail(this.orderId, this.videoFilename, timeInSeconds).subscribe({
      next: (response) => {
        if (response.ok) {
